...
int main(void)
{
	...
	iniciar_logger();
	configServer = leer_config("/home/utnso/workspace/confBiblioSuse/biblioSuse.cfg");
	char* mensaje;
	int socketServidor = getSocketServidor(configServer);
	...
	while(1)
	{
		...
		int socketCliente =  getSocketCliente(socketServidor);
